...
@MainActor
public final class MedicationLogRepository: MedicationLogRepositoryProtocol {
    ...
    public func fetch(medicationId: UUID, date: Date, timeSlotID: UUID) async throws -> MedicationLog? {
        let start = Calendar.current.startOfDay(for: date)
        let end = Calendar.current.date(byAdding: .day, value: 1, to: start) ?? start
        let descriptor = FetchDescriptor<MedicationLogModel>(
            predicate: #Predicate {
    ...
    public func upsert(_ log: MedicationLog) async throws {
    ...
    public func deleteLogs(for medicationId: UUID) async throws {
        let descriptor = FetchDescriptor<MedicationLogModel>(
