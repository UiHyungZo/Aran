...
@MainActor
public final class MenstrualCycleRepository: MenstrualCycleRepositoryProtocol {
    ...
    public func save(_ cycle: MenstrualCycle) async throws {
        context.insert(MenstrualCycleMapper.toModel(cycle))
        try context.save()
    }

    public func update(_ cycle: MenstrualCycle) async throws {
        let id = cycle.id
        let descriptor = FetchDescriptor<MenstrualCycleModel>(
