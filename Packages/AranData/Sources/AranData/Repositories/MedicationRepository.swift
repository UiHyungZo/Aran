...
public final class MedicationRepository: MedicationRepositoryProtocol {
    ...
    public func addTimeSlot(_ slot: MedicationTimeSlot, to medicationId: UUID) async throws {
        let descriptor = FetchDescriptor<MedicationModel>(
    ...
    public func removeTimeSlot(_ slotId: UUID, from medicationId: UUID) async throws {
        let descriptor = FetchDescriptor<MedicationModel>(
            predicate: #Predicate { $0.id == medicationId }
        )
        guard let model = try context.fetch(descriptor).first else { return }
    ...
    public func updateTimeSlot(_ slot: MedicationTimeSlot) async throws {
        let slotId = slot.id
