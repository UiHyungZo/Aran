...
public enum CycleRecordMapper {
    public static func toDomain(_ model: CycleRecordModel) -> CycleRecord {
    ...
    public static func toModel(_ entity: CycleRecord) -> CycleRecordModel {
    ...
    public static func decodeEmbryoRecords(_ raw: String) -> [EmbryoRecord] {
        guard let data = raw.data(using: .utf8),
              let dtos = try? JSONDecoder().decode([EmbryoRecordDTO].self, from: data) else { return [] }
        return dtos.map { $0.toEntity() }
    }

    public static func encodeEmbryoRecords(_ records: [EmbryoRecord]) -> String {
