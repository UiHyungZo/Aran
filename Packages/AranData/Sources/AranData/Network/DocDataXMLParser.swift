...
public enum DocDataXMLParser {
    /// 모든 ARTICLE의 제목·본문을 합쳐 반환 (효능·용법 등 전체용)
    public static func extractText(from xml: String) -> String? {
        format(parse(xml))
    }

    /// 제목에 `keyword`가 포함된 ARTICLE만 추출 (예: "경고")
    public static func extractArticles(from xml: String, titleContaining keyword: String) -> String? {
        format(parse(xml).filter { $0.title.contains(keyword) })
    }

    /// 제목에 `keyword`가 포함되지 않은 ARTICLE만 추출
    public static func extractArticles(from xml: String, titleExcluding keyword: String) -> String? {
        format(parse(xml).filter { !$0.title.contains(keyword) })
    }
