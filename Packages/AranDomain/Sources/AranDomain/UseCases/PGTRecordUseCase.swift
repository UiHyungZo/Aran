...
public protocol PGTRecordUseCaseProtocol {
...
public final class PGTRecordUseCase: PGTRecordUseCaseProtocol {
    private let repository: PGTRecordRepositoryProtocol

    public init(repository: PGTRecordRepositoryProtocol) {
        self.repository = repository
    }

    public func fetchAll() async throws -> [PGTRecord] {
        try await repository.fetchAll()
    }

    public func fetch(cycleRecordId: UUID) async throws -> [PGTRecord] {
        try await repository.fetch(cycleRecordId: cycleRecordId)
    }

    public func fetch(id: UUID) async throws -> PGTRecord? {
    ...
    public func update(_ record: PGTRecord) async throws {
