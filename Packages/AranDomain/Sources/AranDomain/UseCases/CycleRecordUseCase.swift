...
public protocol CycleRecordUseCaseProtocol {
...
extension CycleRecordUseCaseProtocol {
    public func addEvent(_ event: DayEvent, to date: Date) async throws {
        try await addEvent(event, to: date, cycleNumber: 1)
    }
}

public final class CycleRecordUseCase: CycleRecordUseCaseProtocol {
    private let repository: CycleRecordRepositoryProtocol

    public init(repository: CycleRecordRepositoryProtocol) {
    ...
    public func addEvent(_ event: DayEvent, to date: Date, cycleNumber: Int = 1) async throws {
    ...
    public func removeTransferEvent(transferID: UUID) async throws {
    ...
    public func saveDiary(emoji: String?, text: String, for date: Date) async throws {
    ...
    public func clearDiary(for date: Date) async throws {
        guard var existing = try await repository.fetch(date: date) else { return }
        existing.diary = nil
        try await repository.update(existing)
    }

    public func estimateOvulation(from periodStart: Date, cycleLength: Int = 28) -> Date {
